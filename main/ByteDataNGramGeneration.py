...
def main():
    ...
    #args[0]= PreProcessed parquet file for Byte Testing Data
    byteTestParque = spark.read.parquet(args[0])    													                                                                                  
   
    #Unigram
    inputTestN1gram = spark.createDataFrame(byteTestParque, schema)

    cvTest = CountVectorizer(inputCol="n-grams", outputCol="features", vocabSize=256, minDF=1.0,minTF=2.0)
    model1Test = cvTest.fit(inputTestN1gram)
    featurizedTest1Data = model1Test.transform(inputTestN1gram).select("hashcodefile","label","opcodeFeatures")
    featurizedTest1Data.write.parquet("byte1GramFinalTesting.parquet")
  
    #2Gram
    n2gramTestDataRDD=createNgram(byteTestParque,2)
    inputTestN2gram = spark.createDataFrame(n2gramTestDataRDD, schema)

    cvTest1 = CountVectorizer(inputCol="n-grams", outputCol="features", vocabSize=65536, minDF=1.0,minTF=2.0)
    model2Test = cvTest1.fit(inputTestN2gram)
    featurizedTest2Data = model2Test.transform(inputTestN2gram).select("hashcodefile","label","opcodeFeatures")
    featurizedTest2Data.write.parquet("byte2GramFinalTesting.parquet")

    #3Gram
    n3gramTestDataRDD=createNgram(byteTestParque,3)
    inputTestN3gram = spark.createDataFrame(n3gramTestDataRDD, schema)

    model3Test = cvTest1.fit(inputTestN3gram)
    featurizedTest3Data = model3Test.transform(inputTestN3gram).select("hashcodefile","label","opcodeFeatures")
    featurizedTest3Data.write.parquet("byte3GramFinalTesting.parquet")

    #4Gram
    n4gramTestDataRDD=createNgram(byteTestParque,4)
    inputTestN4gram = spark.createDataFrame(n4gramTestDataRDD, schema)

    model4Test = cvTest1.fit(inputTestN4gram)
    featurizedTest4Data = model4Test.transform(inputTestN4gram).select("hashcodefile","label","opcodeFeatures")
    ...
    #args[1]= PreProcessed parquet file for Byte Training Data
    byteTrainParque=spark.read.parquet(args[1])

    # Unigram
    inputTrainN1gram = spark.createDataFrame(byteTrainParque, schema)

    cv = CountVectorizer(inputCol="n-grams", outputCol="content", vocabSize=256, minDF=1.0,minTF=2.0)
    ...
    #2gram
    n2gramTrainDataRDD=createNgram(byteTrainParque,2)
    inputTrainN2gram = spark.createDataFrame(n2gramTrainDataRDD, schema)

    cv1 = CountVectorizer(inputCol="n-grams", outputCol="content", vocabSize=65536, minDF=1.0,minTF=2.0)
    ...
    #3 gran
    n3gramTrainDataRDD=createNgram(byteTrainParque,3)
    ...
    # 4 gram
    n4gramTrainDataRDD=createNgram(byteTrainParque,4)
