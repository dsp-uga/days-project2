...
def main():
    ...
    fieldsDF = [StructField("hashcodefile", StringType(), True), StructField("label", StringType(), True),
                StructField("3-grams", StringType(), False)]
    ...
    ngram = NGram(n=3, inputCol="content", outputCol="3-grams")
    ngramDataFrame = ngram.transform(schemaByteParque).select("hashcodefile","label","3-grams")
    print ("3 N-gram approach completed")
    ngramRDD=ngramDataFrame.rdd
    ngramRDDUnique = ngramRDD.map(lambda line: (line[0].encode('utf-8'),line[1], uniqueByte(line[3])))
    print ("unique 3 N-Grams extracted")
    ...
    ngramDataFrameStringRDD=ngramRDDUnique.map(lambda line:(line[1].encode('utf-8'),' '.join(str(x) for x in line[3])))
    print ("Array type to String Type conversion completed")
    fieldTF = [StructField("label", StringType(), True), StructField("3-grams", StringType(), True)]
    ...
    count=ngramRDDUnique.flatMap(lambda line:(' '.join(str(x) for x in line[3])).split(" ")).distinct().count()
    print ("Count:",count)
